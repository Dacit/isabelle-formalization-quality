...
import isabelle.*
import isabelle.Export.Session_Context
...
class Importer_Service(repo: Graph_Repository, progress: Progress = new Console_Progress()) {
  ...
  private def to_position_entity_nodes(
    session_name: String,
    theory: Theory_Node,
    entities: Iterator[Entity[_]]
  ): Map[Pos_Node, Seq[Entity_Node]] = {
    entities
      ...
      .map {
        case (pos, entities) =>
          val entity_nodes = entities.map { entity =>
            val entity_id = to_entity_uid(
              session_name, Entity_Kind.from_string(entity.export_kind), entity.name)
  ...
  def import_sessions(sessions: List[Import]): Unit = {
    ...
    } using(session_structure.provider()) { session_context =>
      val session = Export_Theory.read_session(session_context)
      session_structure.theory_names.foreach { theory_name =>
        val theory_node = to_theory_node(session_node, theory_name)
        repo.save_theory(theory_node)

        val theory = session.theory(theory_name).getOrElse(
          error("Theory not found: " + theory_name))
        ...
        val position_entity_nodes = to_position_entity_nodes(
          session_structure.session_name,
          theory_node, theory.entity_iterator)
        edge_importer_builder.add_entities(
          theory_name,
          theory.entity_iterator.filter(e => Entity_Kind.is_value(e.export_kind))
            .map(e => (Entity_Kind.from_string(e.export_kind), e.name)))
    ...
    for (session_structure <- sessions) using(session_structure.provider()) { session_context =>
      val session = Export_Theory.read_session(session_context)
      session_structure.theory_names.foreach { theory_name =>
        val theory = session.theory(theory_name).getOrElse(
          error("Theory not found: " + theory_name))
