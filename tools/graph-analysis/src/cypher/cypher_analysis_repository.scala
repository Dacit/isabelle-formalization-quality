package isabelle.graphanalysis.cypher


import isabelle.{Time, error}
import isabelle.graphanalysis.Analysis.Score.{Clustering_Coefficient, Degree_Centrality}
import isabelle.graphanalysis.Analysis.{Frequency, Graph, Score, Scoring}
import isabelle.graph.Filter
...
import isabelle.graph.cypher.Neo4j_Data_Repository
...
import isabelle.graphanalysis.Analysis_Repository
...
class Cypher_Analysis_Repository(data_repo: Neo4j_Data_Repository) extends Analysis_Repository {
  ...
  private def score_query(projection: String, metric: Score): String = metric match {
    case Score.Closeness_Centrality(harmonic) =>
      val method = if (harmonic) "alpha.closeness.harmonic" else "beta.closeness"
      s"$CALL gds.$method.stream({$projection}) $YIELD $NODE_ID, centrality $AS $SCORE"
  ...
  override def store_subgraph(name: String, nodes: Node, to_edge: Linear_Node, edges: Edge_Props): Unit = {
    ...
    val proj_q = s"$CALL gds.graph.project.cypher('$name', $node_q, $rel_q, { parameters: $par_q })"
  ...
  override def fractal_dimension(name: String, nodes: Node): List[(Int, Int)] = {
    ...
    val proj_q = s"""
      $CALL gds.graph.project.cypher('$name', $node_q, "
    ...
    def N_b(l_b: Int): Option[Int] =
    {
      ...
      data_repo.run(query(
        s"$CALL gds.beta.graph.project.subgraph('${name + l_b}', '$name', '*', 'r.distance >= ${l_b.toDouble}')"))
  ...
  override def store_theory_graph(theory: String): Unit = {
    val name = s"pos_directed_$theory"
    data_repo.run(query(s"$CALL gds.graph.drop('$name', false)"))
    if (data_repo.run(query(
      s"""
          MATCH (n:Position)-[:in]->(t:Theory)
        WHERE t.id = "$theory"
        RETURN DISTINCT {} AS exists
""")).flatMap(_.list().asScala).size > 0) {
      data_repo.run(query(
        s"""
