...
class Analysis_Service(analysis_repo: Analysis_Repository, graph_repo: Graph_Repository) {
  ...
  def store_subgraph(name: String, kind: Node_Kind.Value, sessions: Seq[UId], directed: Boolean = true): Unit = {
    val props = Edge_Props(None, directed = directed)
    analysis_repo.store_subgraph(name, node(kind, sessions), linear_node(kind), props)
  }
