...
import Analysis.Score._
import Analysis.{Distribution, Scoring}
import cypher.{Cypher_Analysis_Repository}

import isabelle.graph._
import isabelle.graph.Filter.Dir
import isabelle.graph.Graph.{Entity_Kind, Node_Kind, UId}
import isabelle.graph.cypher.{Cypher_Graph_Repository, Neo4j_Data_Repository}
...
object Graph_Analysis {
  ...
  val store = new Aspect {
    override def name = "store"
    override def process(args: Aspect_Args): Unit =
      args.theory_names.foreach(args.analysis_repo.store_theory_graph)
    override def finish(output_dir: Path): Unit = ()
  }
  ...
  val known_aspects = List(graph, top_prediction, top_concepts, evaluation, stats, detail_stats,
    ...
    average_path_length, indegree_distribution, theory_fractal_dim, store)
  ...
  def graph_analysis(
    ...
    theories: Map[String, List[String]],
    ...
    sessions.foreach { session_name =>
      ...
      val args = Aspect_Args(session_name, output_dir, input_dir, List(session_name),
        theories(session_name), graph_repo, analysis_repo, dirs, progress)
...
""",
    ...
    progress.interrupt_handler {
      ...
      def exported_theories(session_name: String): List[String] = {
        val exported =
          using(Export.open_session_context0(store, session_name))(_.theory_names()).toSet
        deps(session_name).proper_session_theories.map(_.theory).filter(exported.contains)
      }

      val sessions = sessions_structure.build_selection(selection)
      val theories =
        sessions.map(session_name => session_name -> exported_theories(session_name)).toMap

      using(Cypher_Graph_Repository(neo4j)) { graph_repo =>
        val analysis_repo = Cypher_Analysis_Repository(neo4j)
        graph_analysis(aspects = aspects, sessions = sessions, theories = theories,
          graph_repo = graph_repo, analysis_repo = analysis_repo, input_dir = input_dir,
...
class Graph_Analysis_Tools extends Isabelle_Scala_Tools(
  Graph_Analysis.isabelle_tool)
